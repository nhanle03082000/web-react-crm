...
import CompanyTypesForm from './components/CompanyTypesForm';

const CompanyTypes: React.FC = () => {
  const { t } = useTranslation();
  const path = API_URL.COMPANYTYPES;
  const page = t('namepage.loaihinhdoanhnghiep');
            ...
            <Row justify={'space-between'}>
              ...
              <Col span={12}>
                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                  ...
                  {permission.create && (
                    <Create>
                      <CompanyTypesForm isEditing={false} />
                    </Create>
                  )}
                </div>
              </Col>
          ...
          <Card padding="1rem">
            {permission.index && (
              <Show param={param} colums={column} permission={permission}>
                <CompanyTypesForm isEditing={true} />
              </Show>
            )}
          </Card>
        </Col>
