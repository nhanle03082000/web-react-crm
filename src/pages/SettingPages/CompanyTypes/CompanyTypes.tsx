...
const CompanyTypes: React.FC = () => {
  const path = API_URL.COMPANYTYPES;
  ...
  return (
    <>
      <PageTitle>{t('namepage.loaihinhdoangnhiep')}</PageTitle>
      <Crud
        ...
        state={state}
        getDataModal={''}
        namePage={t('namepage.loaihinhdoangnhiep')}
      >
        {loading ? (
          <CustomLoading />
        ) : (
          <>
            <Form.Item name="code" label="Mã loại hình">
              <Input placeholder="Nhập mã loại hình" size="small" disabled={isEditing} />
            </Form.Item>
            <Form.Item name="name" label="Tên loại hình">
              <TextArea rows={4} placeholder="Nhập tên loại hình" size="small" />
            </Form.Item>
