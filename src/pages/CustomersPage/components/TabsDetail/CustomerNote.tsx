...
const CustomerNote: React.FC<IProps> = ({ id }) => {
  ...
  const [filter, setFilter] = useState<IFilter>({
    ...
    total: 0,
    sort_direction: 'desc',
    sort_column: 'customer_notes.createdAt',
  });
  ...
  const columns: ColumnsType = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        return (
          <Space>
            <UpdateModal path={path} page="ghi chú" getList={getCustomerContactsList} id={id} initValue={record}>
              <FormNote isEditing={true} />
            </UpdateModal>
            <Popconfirm
  ...
  return (
    <>
      ...
      <CreateModal path={path} page="ghi chú" getList={getCustomerContactsList} id={id}>
        <FormNote isEditing={false} />
      </CreateModal>
    </>
  );
};
